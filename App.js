...
const STORAGE_KEY = 'askvicky_history';

const SYSTEM_PROMPT = `You are AskVicky — a highly capable, friendly AI assistant for SDCC Bank (The Satara District Central Co-operative Bank) colleagues in Satara, Maharashtra, India. You work exactly like Claude AI and can help with absolutely any topic:
...
const QUICK_CHIPS = [
  ...
  { id: '5', icon: '✉️', label: 'Email',        prompt: 'Write a professional email to the manager requesting one day leave' },
  { id: '6', icon: '🌏', label: 'Translate',    prompt: 'Translate to Marathi: Good morning, how are you? I need your help with this work.' },
  { id: '7', icon: '⚙️', label: 'Shell Script', prompt: 'Write a shell script to take Oracle database backup and send email alert' },
  { id: '8', icon: '📊', label: 'Interest',     prompt: 'Calculate compound interest on Rs 5,00,000 for 3 years at 8.5% per annum' },
];
  ...
  async function sendMessage(textOverride) {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        content: '⚠️ Network error. Please check your internet and try again.',
      }]);
    }
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLogo}>
          <Text style={{ fontSize: 18, color: C.bg }}>✦</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>AskVicky</Text>
          <Text style={styles.headerSub}>AI Assistant · Any Topic</Text>
        </View>
        ...
        {showWelcome ? (
          <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.welcomeContent}>
            ...
            <Text style={styles.welcomeDesc}>
              Your AI assistant for any topic — SQL, banking, calculations, coding, writing, and much more!
            </Text>
...
const styles = StyleSheet.create({
  ...
  headerLogo:   { width: 36, height: 36, borderRadius: 10, backgroundColor: C.accent, alignItems: 'center', justifyContent: 'center' },
  headerTitle:  { fontSize: 15, fontWeight: '700', color: C.text },
  headerSub:    { fontSize: 11, color: C.textDim, marginTop: 1 },
  onlinePill:   { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(34,197,94,0.1)', borderWidth: 1, borderColor: 'rgba(34,197,94,0.25)', borderRadius: 20, paddingHorizontal: 9, paddingVertical: 4 },
  onlineDot:    { width: 6, height: 6, borderRadius: 3, backgroundColor: C.accent },
  onlineText:   { fontSize: 10, color: C.accent, fontWeight: '600' },
  clearBtn:     { backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, borderRadius: 7, paddingHorizontal: 10, paddingVertical: 5 },
  clearBtnText: { fontSize: 11, color: C.textDim },
  ...
  bubble:       { maxWidth: '78%', borderRadius: 14, padding: 12, borderWidth: 1 },
  ...
  welcomeDesc:  { fontSize: 13, color: C.textMid, textAlign: 'center', lineHeight: 20, maxWidth: 300, marginBottom: 24 },
